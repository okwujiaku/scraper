"""
Discord self-bot — multi-account join scraper with smart routing.

Watches third-party log-bot "New Member Joined!" messages and forwards them to
the correct brand group chat. Multiple accounts run in one Render worker via
TOKEN_CLIENT_1..N and CHAT_ID_CLIENT_1..N.

Any account may see a join log; the router picks the output group by server
membership (Clients 4→3→2→1 beat Client 1 on shared servers). Override with
MONITOR_SERVERS_CLIENT_N=Server A,Server B on Render.
...
class ScraperClient(discord.Client):
    ...
    async def on_ready(self):
        ...
        if not self.target_chat_id:
            print(
                f"[{self.name}] No target chat ID configured; will capture but not forward.",
                flush=True,
            )
            return
        ...
        try:
            ...
            print(
                f"[{self.name}] Forwarding captures to: {self._target_channel} "
            ...
            print(
                f"[{self.name}] Could not open chat {self.target_chat_id}: {exc}",
                flush=True,
            )
